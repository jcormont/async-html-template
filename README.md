...
By default, HTML output is minified (using the NPM package `html-minifier`). You can supply minification options to the `renderAsync` method of the `Template` class, _or_ use the exported `defaultHtmlMinifierOptions` object which contains default options.
...
If you do not specify a context expression, a shallow clone of the current context object will be used as the context for the included partial.
